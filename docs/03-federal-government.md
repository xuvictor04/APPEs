# 03 — Federal & Government APPEs

Back to [index](../README.md).

The most competitive and most distinctive non-clinical rotations in pharmacy. Nearly all
require **U.S. citizenship**, all require long onboarding lead times, and almost all are
unpaid academic credit (the COSTEP externships are the paid exception).

---

## FDA — Pharmacy Student Experiential Program (PSEP) ❓ status unresolved

**Location:** Silver Spring, MD (White Oak) and other FDA sites
**Duration:** 4–6 weeks, offered year-round
**Email:** **Pharmacy.Student@fda.hhs.gov**
**Phone:** (855) 543-3784 toll free · (301) 796-3400
**Application portal:** [FDA PSEP online application](https://www.accessdata.fda.gov/scripts/FDASpeaker/Students.cfm)
**Links:** [PSEP overview](https://www.fda.gov/about-fda/scientific-internships-fellowships-trainees-and-non-us-citizens/fda-pharmacy-student-experiential-program) ·
[How to apply](https://www.fda.gov/about-fda/fda-pharmacy-student-experiential-program/how-apply) ·
[About PSEP](https://www.fda.gov/about-fda/fda-pharmacy-student-experiential-program/about-fda-psep) ·
[Program history](https://www.fda.gov/about-fda/fda-pharmacy-student-experiential-program/program-history) ·
[Information for placed students](https://www.fda.gov/about-fda/fda-pharmacy-student-experiential-program/information-placed-students)

### ❓ Read this before writing FDA off

FDA's pages carry a notice that APPE opportunities are unavailable and that FDA is **"not
accepting applications for the 2026–2027 academic year at this time,"** adding that
**"Experiential Coordinators will be notified directly when the application process reopens."**

**That notice is labeled 2026–27. If you're targeting 2027–28 rotations, it does not
necessarily describe your cycle.** Two readings are possible and I could not resolve which is
right — fda.gov is unreachable from the environment these docs were researched in:

1. **The notice is last cycle's text**, left in place. In that case applications for 2027–28
   may be open **right now** — see the timing below.
2. **The program is still paused.** The "will be notified when it reopens" phrasing leans this
   way, but it doesn't name a year, so it doesn't settle the question either.

### Why this is urgent either way

PSEP historically accepts applications **in the fall preceding your final year — i.e., at the
start of P3.** A previous cycle's window ran **September 1 – October 31.**

You are a P3 in September 2026 targeting 2027–28 rotations. **If PSEP is open for your cycle,
that window is open today.** Do not wait for a posted announcement.

### What to do this week

1. **Ask your experiential coordinator.** They are the notification channel — FDA says
   coordinators are contacted directly when applications reopen. Your coordinator may already
   have the notice sitting in their inbox. This is the highest-yield step, and it costs one email.
2. **Email Pharmacy.Student@fda.hhs.gov** and ask plainly: *are applications open for the
   2027–2028 academic year, and if not, when do you expect them to reopen?*
3. **Check the [online application portal](https://www.accessdata.fda.gov/scripts/FDASpeaker/Students.cfm)** —
   if it accepts an entry, the cycle is live regardless of what the static page says.
4. **Note the process:** students apply **with support from their school's experiential
   coordinator**, not independently. Loop them in first, not last.

**Eligibility:** U.S. citizenship, enrollment in an ACPE-accredited PharmD program, eligible
to begin APPE rotations.

**What you'd do:** rotations sit across FDA's centers and offices — CDER (drug evaluation),
CBER (biologics), CDRH (devices), the Office of Clinical Pharmacology, drug safety and
pharmacovigilance, regulatory review, and policy offices.

### Backup routes into FDA

Worth pursuing in parallel, not only if PSEP is closed:

- **CDER Student Volunteer Service Program (SVSP)** — PSEP has historically sat inside SVSP.
- **ORISE fellowships** —
  [FDA scientific fellowships and training programs in CDER](https://www.fda.gov/about-fda/jobs-center-drug-evaluation-and-research-cder/scientific-fellowships-and-training-programs-cder).
  ORISE support has been available for pre-doctoral students in the Office of Clinical
  Pharmacology. Not an APPE, but the same building and the same network.
- **JRCOSTEP / SRCOSTEP** — FDA is a participating agency (see USPHS below).

## NIH Clinical Center — Pharmacy Department ⚠️ status corrected

**Location:** Bethesda, MD
**Status (verified July 2026):** **direct department inquiry / school-arranged.**
**No current public APPE application or deadline verified.**
**Phone:** 301-496-4363
**Link:** [NIH Clinical Center Pharmacy Department](https://www.cc.nih.gov/pharmacy) ·
[clinical electives](https://www.cc.nih.gov/training/students/clinical_electives)

> ⚠️ **Correction.** An **October 31 deadline** circulates widely for this program — I had it
> in an earlier draft of these docs. The July 2026 link-verified review found that *"the prior
> October 31 deadline and old PDF were not confirmed on the current official department page."*
> Do not build your calendar around that date.

**How to pursue instead:** have your **experiential office** contact the Pharmacy Department
directly to ask (a) whether visiting APPE placements are available for the cycle you want, and
(b) whether an affiliation agreement can be established. This is an OEE-to-department
conversation, not a student application.

**Program context:** the NIH Clinical Center Pharmacy supports approximately **1,000 clinical
studies** and manages both commercial and investigational medications — the investigational
drug work is the distinctive part, and it's hard to get anywhere else at this scale.

**Rotation areas historically described:** administration; drug information / drug policy;
pharmacy informatics; inpatient and outpatient operations; and pharmacokinetics / clinical
research / clinical pharmacology.

**International students:** NIH has historically accommodated them with extra processing time
and real cancellation risk. Apply early and keep a backup.

**Adjacent NIH options:** the National Cancer Institute and other institutes host research
trainees; NIH's IRTA/postbac programs are a separate, non-APPE pathway.

## CDC — Pharmacy student rotations ✅

**Location:** Atlanta, GA (some placements elsewhere)
**Status (verified July 2026):** **no fixed public deadline — email for current cycle
information.**
**Email:** **pharmacists@cdc.gov**
**Link:** [CDC Doctoral and Medical Student Short-Term Opportunities](https://www.cdc.gov/careerpaths/students/index.html) ·
[CDC rotations — USPHS Pharmacist PAC](https://dcp.psc.gov/OSG/pharmacy/cdc.aspx)

- **Eligibility:** fourth-year pharmacy students (PY4)
- **Experience:** **4–6 weeks, unpaid**
- **Placements may include:** Drug Services, Regulatory Affairs, HIV/AIDS prevention,
  respiratory disease programs, and emergency operations
- **How to pursue:** email the CDC pharmacy program and **copy or involve your experiential
  office early**

> A USPHS committee page describes a September-open / January 1 deadline. The July 2026
> verification found no fixed public deadline. **Email rather than wait** — and if you get a
> date from CDC directly, correct it in [02](02-timeline-and-deadlines.md).

Strong pairing with an ID or ambulatory care rotation if you're building a public health story.

## Veterans Affairs (VA)

**Locations:** ~170 medical centers nationwide — the largest single employer of pharmacists
in the U.S.
**Duration:** typically 3–7 weeks, full-time
**Link:** each facility's own page under [va.gov](https://www.va.gov/) → *Work with us →
Internships and fellowships*

**Rotation areas commonly available:** acute care, ambulatory care (VA pharmacists have
broad scope — many run their own clinics under scope of practice agreements), geriatrics,
mental health, pain management, pharmacoeconomics, and institutional practice.

**How to apply:** VA is decentralized. There is no national APPE portal. Find the facility,
find its pharmacy education or residency page, and follow the local registration process —
usually coordinated through your school's OEE. Onboarding (background check, badging,
occupational health) is the slow part; start 4–6 months out.

**Why VA is worth targeting:** ambulatory care scope of practice is unusually advanced, the
EHR gives students real access, and a VA APPE is a well-understood credential if you're
aiming at a VA residency later.

**Northeast/Mid-Atlantic VA sites to look at:** see
[09 — Northeast & Mid-Atlantic Sites](09-northeast-mid-atlantic-sites.md).

> Note: **VALOR** (VA Learning Opportunities Residency) is a *nursing* program. It comes up
> constantly in searches for VA student programs and is not a pharmacy pathway.

---

## Indian Health Service (IHS)

**Locations:** IHS direct-service facilities across ~39 states; heavy presence in the
Southwest (AZ, NM, OK), the Northern Plains, and Alaska
**Link:** [IHS Pharmacy student opportunities](https://www.ihs.gov/pharmacy/studentops/) ·
[IHS student opportunities](https://www.ihs.gov/careeropps/studentopportunities/) ·
[Residencies and rotations](https://www.ihs.gov/careeropps/studentopportunities/residencies-and-rotations/)

IHS has collaborative agreements with universities for its APPE program; students from
**80+ universities in 39 states** complete experiential training at IHS facilities.

**How to apply:** Start with your school's experiential coordinator — confirm your school
has an agreement with your target site. If not, your coordinator (not you) contacts the IHS
Pharmacy Program. A current list of rotation sites and contacts is downloadable from the IHS
Pharmacy site under *IHS Rotations/Clerkships*.

**Terms:** unpaid, for academic credit, final-year students. **You pay travel and housing** —
budget for it, since many sites are genuinely remote.

**Why it's a standout rotation:** IHS pharmacists practice at the top of their license, often
with prescriptive authority and independent clinics. It's one of the few places a student
sees pharmacist-run primary care as the norm.

**Paid IHS options:** JRCOSTEP, SRCOSTEP, and Senior Year Externship Rotations.

---

## USPHS Commissioned Corps — JRCOSTEP & SRCOSTEP

**Link:** [usphs.gov/students](https://www.usphs.gov/students/) ·
[JRCOSTEP FAQs (PDF)](https://www.usphs.gov/media/yunnlaiu/jrcostep-faqs.pdf)

These are **paid** externships, not APPEs — but they're the most direct route into federal
pharmacy and they feed the same networks.

| | JRCOSTEP | SRCOSTEP |
| --- | --- | --- |
| Who | Students with ≥1 year of school remaining (P2/P3) | Students in their final year |
| Length | 31–120 days, during school breaks | Longer commitment |
| Reported window | **July 1 – September 15** | Typically winter/spring |
| Pay | Yes — active duty pay/allowances | Yes, with a service obligation |

**Participating agencies:** FDA, IHS, CMS, CDC, National Park Service, and the Program
Support Center. That list is itself a useful map of where federal pharmacists work.

**Eligibility:** U.S. citizenship, age and physical-fitness standards, good academic standing.

---

## Other federal and quasi-federal targets

These don't all run advertised APPE programs — several are cold-outreach targets. Worth
the email.

| Agency | Angle | Link |
| --- | --- | --- |
| **CMS** (Baltimore, MD) | Medicare Part D, drug pricing, coverage policy. Participates in COSTEP. | [cms.gov](https://www.cms.gov/) |
| **DEA** (Arlington, VA) | Diversion control, controlled-substance policy | [dea.gov](https://www.dea.gov/) |
| **AHRQ** (Rockville, MD) | Health services research, patient safety | [ahrq.gov](https://www.ahrq.gov/) |
| **NCI** (Bethesda/Frederick, MD) | Oncology drug development, clinical trials | [cancer.gov](https://www.cancer.gov/) |
| **BARDA / ASPR** (Washington, DC) | Medical countermeasures, preparedness | [aspr.hhs.gov](https://aspr.hhs.gov/) |
| **DoD / military treatment facilities** | Walter Reed (Bethesda), Fort Belvoir, Naval hospitals | facility-specific |
| **Bureau of Prisons** | Correctional pharmacy; participates in federal student programs | [bop.gov](https://www.bop.gov/) |
| **State health departments** | Immunization programs, opioid response, 340B, Medicaid pharmacy | state-specific |
| **State boards of pharmacy** | Regulation, inspection, licensure — see [NABP](https://nabp.pharmacy/) for the board directory |

---

## Practical notes for every federal application

- **Citizenship is usually non-negotiable.** Confirm before investing effort.
- **Onboarding is the bottleneck.** Background investigation, fingerprinting, and badging
  routinely take 2–4 months *after* acceptance. A late acceptance can still fail on logistics.
- **Affiliation agreements with federal agencies are slow.** Ask your OEE in September, not
  in March.
- **Unpaid + expensive location.** Bethesda, Silver Spring, and DC are costly. Ask your school
  about experiential travel stipends; several exist and go unclaimed.
- **Apply to more than one.** FDA, NIH, and CDC each take a small cohort. Treat them as
  lottery tickets you buy in bulk, with a solid non-federal backup behind them.

---

Next: [04 — Industry & Regulatory](04-industry-and-regulatory.md)
